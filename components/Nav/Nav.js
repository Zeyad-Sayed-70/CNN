...
export const Nav = ({ articles }) => {
  ...
  const handlerSearch = (ev) => {
    const newArticles = articles.filter((article) => {
      if (
        article.title?.toLowerCase()?.includes(ev.target.value.toLowerCase())
      ) {
        return true;
      }

      if (
        article.author?.toLowerCase()?.includes(ev.target.value.toLowerCase())
      ) {
        return true;
      }
        ...
        <Container className="d-flex align-center justify-content-between py-3">
          <Col className="d-flex gap-3 col-6 col-md-9">
            ...
            <div className={styles.logo}>
              <Link href="/">
                <img src="cnn_icon.png" alt="logo/cnn" />
              </Link>
            </div>
          ...
          <div className={`${styles.searchContainer}`}>
            ...
            >
              ...
              {searchData?.map((data) => (
                <li key={data.title} className={`${styles.searchList}`}>
                  <a href={`#${data.title}`}>{data.title}</a>
                </li>
              ))}
            </div>
            <div
              className={isSearchList ? styles.overlay : ""}
              onClick={() => {
                setIsSearchList(false);
              }}
            ></div>
